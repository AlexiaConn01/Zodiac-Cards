...
extern MY_HIGHP_OR_MEDIUMP vec2 prismatic_shader;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float t = prismatic_shader.y*5.221 + time;
    ...
    float field = (1.+ (
        cos(length(field_part1) / 19.483) + sin(length(field_part2) / 33.155) * cos(field_part2.y / 15.73) +
        cos(length(field_part3) / 27.193) * sin(field_part3.x / 21.92) ))/2.;
    float res = (.5 + .5* cos( (prismatic_shader.x) * 2.612 + ( field + -.5 ) *3.14));
	...
	hsl.x = hsl.x + res + (fac + prismatic_shader.y*0.04)/2.;


	hsl.y = (hsl.y*1.3 + min(0.6,hsl.y+0.5))/2;
